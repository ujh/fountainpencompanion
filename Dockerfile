...
RUN --mount=type=secret,id=HONEYBADGER_API_KEY,required=false \
  SECRET_KEY_BASE_DUMMY=1 PRECOMPILING_ASSETS=true HONEYBADGER_REPORT_DATA=false \
  HONEYBADGER_API_KEY="$(cat /run/secrets/HONEYBADGER_API_KEY 2>/dev/null)" \
