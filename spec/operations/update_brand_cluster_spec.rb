...
describe UpdateBrandCluster do
  ...
  it "updates the brand_cluster of the macro cluster" do
    expect do described_class.new(macro_cluster, brand_cluster).perform end.to change {
      macro_cluster.reload.brand_cluster
    }.from(old_brand_cluster).to(brand_cluster)
  end
  ...
  it "updates other macro clusters with the same brand name" do
    ...
    expect do described_class.new(macro_cluster, brand_cluster).perform end.to change {
      second_macro_cluster.reload.brand_cluster
    }.to(brand_cluster)
  end
