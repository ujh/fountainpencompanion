...
describe Unfurler::Html do
  ...
  context "anderson" do
    ...
    it "extracts the image" do
      expect(subject.image).to eq(
        "http://blog.andersonpens.com/wp-content/uploads/2016/05/InkReview_Sailor_Miruai_Full.jpg"
      )
    end
  ...
  context "fpn" do
    ...
    it "extracts the url" do
      expect(subject.url).to eq(
        "https://www.fountainpennetwork.com/forum/topic/217856-de-atramentis-sherlock-holmes/"
      )
    end
    ...
    it "extracts the description" do
      expect(subject.description).to eq(
        "Here is a quick review of De Atramentis Sherlock Holmes, which is called Night Blue. It's a very nice dark blue, fairly wet and with some nice shading. I really like it I'm not entirely happy with the scan. The inks are all slightly more vibrant and darker than they appear here. I'm not sure how ..."
      )
    end

    it "extracts the image" do
      expect(subject.image).to eq(
        "https://www.fountainpennetwork.com/forum/uploads/imgs/fpn_1332727197__sherlock_holmes_1_copy.jpg"
      )
    end
