...
describe CollectedInk do
  ...
  describe "tags" do
    it "adds new tags" do
      ...
      expect do ink.save end.to change { Gutentag::Tag.count }.by(2)
    end
    ...
    it "strips out all empty spaces at the ends" do
      ...
      ink.tags_as_string = " one , two "
      ink.save
      expect(Gutentag::Tag.pluck(:name)).to match_array(%w[one two])
    end
  end
  ...
  describe "#to_csv" do
    ...
    it "has a header row" do
      expect(CollectedInk.none.to_csv).to eq(
        "Brand;Line;Name;Type;Color;Swabbed;Used;Comment;Private Comment;Private;Archived;Archived On;Usage;Tags;Date Added;Maker;Daily Usage;Last Usage\n"
      )
    end
