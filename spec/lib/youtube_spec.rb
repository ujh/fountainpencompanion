  ...
  describe '#fetch_videos' do
    let(:client) do
      double(
        ...
        list_channels: double(
          items: [double(
            content_details: double(
              related_playlists: double(uploads: 'uploads_id')
            )
          )]
        ),
    ...
    let(:list_playlist_items) do
      ...
      video1 = double(:video, snippet: double(resource_id: double(video_id: 'video1'), title: 'title1'))
      video2 = double(:video, snippet: double(resource_id: double(video_id: 'video2'), title: 'title2'))
