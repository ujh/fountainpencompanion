// @ts-check
import { fuzzyMatch, createFuzzyMatch, fuzzyFilter } from "./match";
...
describe("fuzzyFilter with composite filterValue", () => {
  ...
  it("falls back to plain string filterValue for backward compat", () => {
  ...
  it("behaves the same as createFuzzyMatch for equivalent inputs", () => {
    const matchWithClosure = createFuzzyMatch(["comment"]);
    expect(fuzzyFilter(input[0], null, { text: "pilot", hiddenFields: ["comment"] })).toBe(
      matchWithClosure(input[0], null, "pilot")
    );
    expect(fuzzyFilter(input[0], null, { text: "vivid", hiddenFields: ["comment"] })).toBe(
      matchWithClosure(input[0], null, "vivid")
    );
  });
});
